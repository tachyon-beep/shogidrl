...
def test_evaluate_current_agent_real_integration(tmp_path):
    ...
    from keisei.evaluation.core_manager import EvaluationManager
    from keisei.evaluation.core import SingleOpponentConfig, EvaluationStrategy, EvaluationResult

    cfg = SingleOpponentConfig(
        num_games=1,
        opponent_name="test_opponent"
    )
    manager = EvaluationManager(cfg, run_name="real_integration", pool_size=1, elo_registry_path=None)
    # Setup runtime
...
def test_in_memory_evaluation_full_workflow(tmp_path):
    ...
    from tests.evaluation.factories import EvaluationTestFactory
    from keisei.evaluation.core_manager import EvaluationManager
    from keisei.evaluation.core import create_evaluation_config, EvaluationStrategy, EvaluationResult
    ...
    manager = EvaluationManager(cfg, run_name="in_memory_flow", pool_size=1, elo_registry_path=str(tmp_path / "elo.json"))
    ...
    result = asyncio.run(manager.evaluate_current_agent_in_memory(test_agent, str(opp_ckpt)))
    assert isinstance(result, EvaluationResult)
...
def test_evaluation_error_propagation(monkeypatch):
    ...
    from keisei.evaluation.core import EvaluationConfig, EvaluationStrategy
    from tests.evaluation.factories import EvaluationTestFactory
    ...
    monkeypatch.setattr(
        EvaluatorFactory,
        'create',
        lambda cfg: FailingEvaluator(cfg)
    )
    cfg = EvaluationConfig(strategy=EvaluationStrategy.SINGLE_OPPONENT, num_games=1)
    manager = EvaluationManager(cfg, run_name="error_prop", pool_size=1, elo_registry_path=None)
    test_agent = EvaluationTestFactory.create_test_agent()
    manager.setup(device="cpu", policy_mapper=test_agent.config.env, model_dir=".", wandb_active=False)
