...
class TestPerformanceValidation:
    ...
    def test_memory_usage_validation(self):
        ...
        for i in range(15):  # Exceed cache size to test LRU
            weights = {
                f'layer_{j}.weight': torch.randn(128, 64) for j in range(5)
            }
            ...
            with tempfile.NamedTemporaryFile(suffix='.pth', delete=False) as tmp_file:
                ...
                try:
                    weight_manager.cache_opponent_weights(opponent_id, Path(tmp_file.name))
                finally:
        ...
        assert memory_increase < 500, f"Memory usage increased by {memory_increase:.1f} MB, should be under 500 MB"
    ...
    def test_evaluation_manager_throughput(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_agent.model.state_dict.return_value = {'layer.weight': torch.randn(32, 16)}
            ...
            assert avg_time < 10.0, f"Average evaluation time {avg_time:.2f}s should be under 10s"
    ...
    def test_in_memory_evaluation_performance(self):
        ...
        try:
            ...
            assert operation_time < 10.0, f"Agent creation took {operation_time:.2f}s, should be under 10s"
            ...
            operation_time = time.time() - start_time
            assert operation_time < 5.0, f"Weight handling took {operation_time:.2f}s even when failing"
        ...
        for size_name, weights in test_cases:
            ...
            with tempfile.NamedTemporaryFile(suffix='.pth', delete=False) as tmp_file:
                ...
                try:
                    ...
                    cached_weights = weight_manager.cache_opponent_weights(opponent_id, Path(tmp_file.name))
            ...
            assert operation_time < 2.0, f"{size_name} weights took {operation_time:.3f}s, should be under 2s"
