...
class TestParallelGameExecutor:
    ...
    def test_parallel_game_executor_concurrent_execution(
        self, test_isolation, performance_monitor, thread_isolation
    ):
        ...
        assert (
            execution_time < 2.0
        ), f"Parallel execution took {execution_time:.3f}s, should be under 2s"
        ...
        assert (
            agent_a_wins == 4 and agent_b_wins == 4
        ), "Winner distribution should be 50/50"
    ...
    def test_load_balancing_across_workers(self, test_isolation, performance_monitor):
        ...
        assert (
            execution_time < expected_sequential_time * 0.7
    ...
    def test_error_handling_and_fault_tolerance(self, test_isolation, error_injector):
        ...
        assert (
            failed_games > 0
        ), "Should have some failed games (due to 30% failure rate)"

        success_rate = successful_games / total_games
        assert (
            0.4 <= success_rate <= 0.9
        ), f"Success rate {success_rate:.2f} outside expected range"
    ...
    def test_performance_benchmarks(self, test_isolation, performance_monitor):
        ...
        assert parallel_time < 1.0, f"Parallel execution too slow: {parallel_time:.3f}s"
