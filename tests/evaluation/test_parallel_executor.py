...
class TestParallelGameExecutor:
    ...
    def test_parallel_game_executor_concurrent_execution(self):
        ...
        with patch(
            "keisei.evaluation.core.parallel_executor.ParallelGameExecutor"
        ) as MockExecutor:
            ...
            result = executor.execute_games_parallel(
                agent_pairs=[("agent_a", "agent_b")] * 8, max_workers=4
            )
    ...
    def test_load_balancing_across_workers(self):
        ...
        task_counts = [
            len(tasks) for tasks in distribution["worker_assignments"].values()
        ]
