...
class TestPerformanceValidation:
    ...
    def test_memory_usage_limits_validation(self):
        ...
        try:
            ...
            assert memory_usage["total_mb"] < 2000, f"Weight cache uses {memory_usage['total_mb']:.1f} MB, should be under 2000 MB"
            assert metrics["memory_delta_mb"] < 2500, f"Process memory increased by {metrics['memory_delta_mb']:.1f} MB, should be under 2500 MB"
    ...
    def test_agent_creation_performance_benchmark(self, sample_weights):
        ...
        assert metrics["duration_seconds"] < 15.0, f"Agent creation took {metrics['duration_seconds']:.3f}s, should be under 15s"
        
        # Should not leak excessive memory (increased limit for realistic weights)
        assert metrics["memory_delta_mb"] < 400, f"Memory increased by {metrics['memory_delta_mb']:.1f} MB, should be under 400 MB"
    ...
    def test_cache_performance_and_lru_efficiency(self):
        ...
        try:
            ...
            assert metrics["duration_seconds"] < 2.0, f"Cache operations took {metrics['duration_seconds']:.3f}s, should be under 2s"
    ...
    def test_memory_leak_detection(self):
        ...
        assert memory_growth < 500, f"Memory grew by {memory_growth:.1f} MB, indicating potential memory leak"
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i in range(5):  # More evaluations for better measurement
                with patch("keisei.evaluation.strategies.single_opponent.ShogiGame"):
                    ...
                    assert result is not None, f"Evaluation {i} should complete successfully"
            ...
            print(f"Memory usage during evaluations: {metrics['memory_delta_mb']:.1f}MB")

            # Performance requirements
            assert avg_time < 8.0, f"Average evaluation time {avg_time:.2f}s should be under 8s"
            assert metrics["memory_delta_mb"] < 100, f"Memory usage {metrics['memory_delta_mb']:.1f}MB should be under 100MB"
    ...
    def test_gpu_memory_efficiency(self, sample_weights):
        ...
        try:
            ...
            agents = []
            for i in range(10):
                agent = weight_manager.create_agent_from_weights(sample_weights, device=device)
                agents.append(agent)
            ...
            assert gpu_memory_used < 1000, f"GPU memory usage {gpu_memory_used:.1f}MB should be under 1000MB"
            assert metrics["duration_seconds"] < 20.0, f"GPU operations took {metrics['duration_seconds']:.3f}s, should be under 20s"
    ...
    def test_scalability_with_opponent_count(self):
        ...
        for i in range(1, len(timings)):
            ...
            print(f"Scaling from {opponent_counts[i-1]} to {opponent_counts[i]}: {scaling_factor:.2f}x time increase")
            
            # Scaling should be reasonable - allow up to 2x time increase for linear growth
            assert scaling_factor <= opponent_factor * 2.0, f"Scaling factor {scaling_factor:.2f} too high for opponent increase {opponent_factor:.2f}"
    ...
    def test_weight_extraction_performance_stress(self):
        ...
        for _ in range(200):  # 200 extractions
            _ = weight_manager.extract_agent_weights(agent)  # Don't store unused variable
        ...
        assert metrics["duration_seconds"] < 5.0, f"Weight extraction took {metrics['duration_seconds']:.3f}s, should be under 5s"
        
        # Should not use excessive memory
        assert metrics["memory_delta_mb"] < 100, f"Memory increased by {metrics['memory_delta_mb']:.1f}MB during extractions"
