...
class TestPerformanceValidation:
    ...
    def performance_config(self):
        ...
        return create_evaluation_config(
            ...
            strategy_params={"max_moves_per_game": 10},  # Short games in strategy params
        )
    ...
    def test_cpu_utilization_efficiency(self, test_isolation, performance_monitor):
        ...
        with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
            futures = [
                executor.submit(cpu_intensive_extraction, agent) 
                for agent in agents
            ]
        ...
        print(f"CPU operations: {total_operations} in {metrics['duration_seconds']:.3f}s")
        print(f"Operations per second: {operations_per_second:.1f}")
        print(f"CPU cores utilized: {len(agents)}")
        
        # Should achieve reasonable throughput (adjusted for realistic expectations)
        expected_min_ops_per_sec = max(len(agents) * 2, 50)  # At least 2 ops/sec per core or 50 total
        assert operations_per_second >= expected_min_ops_per_sec, \
            f"CPU efficiency too low: {operations_per_second:.1f} ops/sec, expected >= {expected_min_ops_per_sec}"

    def test_comprehensive_speedup_validation(self, isolated_temp_dir, test_isolation, performance_monitor):
        ...
        try:
            ...
            for i in range(50):  # 50 access operations
                ...
                weights = weight_manager._weight_cache[opponent_id]  # Direct access for testing
                total_params = sum(tensor.numel() for tensor in weights.values())
            ...
            assert cache_stats["cache_size"] == len(checkpoint_files), \
    ...
    def test_memory_pressure_and_cleanup(self, test_isolation, memory_monitor):
        ...
        try:
            ...
            for i in range(50):  # Many more than cache size
                ...
                weights = {
                    "large_conv.weight": torch.randn(128, 128, 5, 5),  # Large convolution
                ...
                weight_manager.cache_opponent_weights(f"large_agent_{i}", Path(tmp.name))
            ...
            assert final_stats["cache_size"] == weight_manager.max_cache_size, \
            ...
            assert memory_after_cleanup < 1000, \
