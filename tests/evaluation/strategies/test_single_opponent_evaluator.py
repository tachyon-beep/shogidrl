...
from keisei.evaluation.core import AgentInfo, SingleOpponentConfig, OpponentInfo, EvaluationContext
...
async def test_evaluate_in_memory_basic():
    ...
    with patch.object(evaluator, 'validate_agent', return_value=True), \
         ...
         patch.object(evaluator, 'evaluate_step_in_memory', new_callable=AsyncMock) as mock_evaluate_step, \
...
async def test_load_evaluation_entity_in_memory_fallback():
    ...
    with patch("keisei.evaluation.strategies.single_opponent.load_evaluation_agent") as mock_load:
