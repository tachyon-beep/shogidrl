...
from keisei.config_schema import AppConfig, EvaluationConfig, TrainingConfig # Corrected: TrainingConfig is directly in config_schema
...
@patch(
    "keisei.core.ppo_agent.PPOAgent"  # Ensure this patch target is correct
)
def test_load_evaluation_agent_updates_minibatch_size(mock_ppo_agent_class, policy_mapper, tmp_path):
    ...
    def ppo_agent_constructor_side_effect(*args, **kwargs):
        ...
        if not hasattr(mock_created_agent_instance, 'load_model') or \
           not isinstance(mock_created_agent_instance.load_model, MagicMock):
    ...
    assert isinstance(agent, mock_created_agent_instance.__class__)  # Ensure agent is of correct class
    # The agent.config will be an AppConfig instance, so we check its training part.
    assert agent.config.training.minibatch_size == 2  # Check that minibatch_size is set to 2
