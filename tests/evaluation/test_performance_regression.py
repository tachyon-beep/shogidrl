...
class TestAgentFactory:
    ...
    def create_test_agent(config: AppConfig):
        ...
        return PPOAgent(
            model=model, config=config, device=torch.device(config.env.device)
        )
...
class MockGameResultFactory:
    ...
    def create_successful_game_result(
        winner: str = "agent", game_length: int = 50, game_id: Optional[str] = None
    ):
...
class TestPerformanceRegression:
    ...
    def test_evaluation_throughput_baseline(
        ...
        # Validate throughput
        games_per_second = (
            result.summary_stats.total_games / metrics["duration_seconds"]
        )
        ...
        assert (
            ...
            < PERFORMANCE_BASELINES["memory_per_game"]
            * result.summary_stats.total_games
        ...
        assert (
            result.summary_stats.win_rate >= 0.99
        )  # All mocked as wins (allow for floating point precision)
    ...
    def test_memory_stability_over_time(
        ...
        for cycle in range(5):
            with patch(
                ...
                mock_play.return_value = (
                    MockGameResultFactory.create_successful_game_result(
                        winner=winner, game_length=50 + cycle * 5
                    )
                )
        ...
        assert (
            max_increase < PERFORMANCE_BASELINES["memory_stability_limit"]
        ), f"Memory spike of {max_increase / 1024 / 1024:.1f}MB exceeds stability limit"

        assert (
            final_increase < PERFORMANCE_BASELINES["memory_stability_limit"] // 2
    ...
    def test_in_memory_evaluation_speedup(
        self, test_agent, performance_monitor: PerformanceMonitor
    ):
        ...
        assert (
            ...
            < PERFORMANCE_BASELINES["weight_extraction_time"]
        ...
        assert (
            reconstruction_metrics["duration_seconds"]
            < PERFORMANCE_BASELINES["agent_creation_time"]
        ...
        self._log_performance_metrics(
            "In-Memory Evaluation",
            {
                ...
                "agent_reconstruction_seconds": reconstruction_metrics[
                    "duration_seconds"
                ],
            },
        )
    ...
    def test_cache_performance_and_efficiency(self):
            ...
            with tempfile.NamedTemporaryFile(suffix=".pt", delete=False) as temp_file:
                ...
                try:
                    weight_manager.cache_opponent_weights(
                        f"opponent_{i}", Path(temp_file.name)
                    )
                finally:
        ...
        assert (
            avg_retrieval_time < PERFORMANCE_BASELINES["cache_hit_time"]
    ...
    def test_concurrent_evaluation_overhead(
        ...
        with patch(
            "keisei.evaluation.strategies.tournament.TournamentEvaluator._play_games_against_opponent"
        ) as mock_play:
        ...
        expected_games = len(tournament_config.opponent_pool_config) * (
            tournament_config.num_games_per_opponent or 1
        )
    ...
    def test_enhanced_features_performance_impact(
        ...
        performance_overhead = (
            enhanced_metrics["duration_seconds"] / basic_metrics["duration_seconds"]
        )
        assert (
        ...
        memory_overhead = (
            enhanced_metrics["memory_delta_bytes"] - basic_metrics["memory_delta_bytes"]
        )
        assert (
    ...
    def test_performance_baseline_validation(
        ...
        memory_per_game = (
            metrics["memory_delta_bytes"] / result.summary_stats.total_games
        )
        ...
        baseline_results = {
            ...
            "games_per_second": (
                games_per_second,
                PERFORMANCE_BASELINES["games_per_second"],
            ),
            "memory_per_game": (
                memory_per_game,
                PERFORMANCE_BASELINES["memory_per_game"],
        ...
        assert (
            not failed_baselines
        ), f"Performance baselines failed: {'; '.join(failed_baselines)}"
    ...
    def test_evaluation_timeout_handling(self, tmp_path, eval_config):
        ...
        try:
            ...
            with patch(
                ...
                _ = manager.evaluate_current_agent(
                    test_agent
                )  # Result not needed for timeout test
            ...
            logger.info(
                "Evaluation failed with: %s (acceptable for timeout test)", str(e)
            )
