...
class TestEvaluationErrorHandling:
    ...
    def test_corrupted_checkpoint_recovery(self, tmp_path):
        ...
        assert (
            "failed to load checkpoint file" in error_msg or "checkpoint" in error_msg
        )
        ...
        for config_data in invalid_configs:
            with pytest.raises(Exception) as exc_info:
                ...
                if "invalid_strategy" in str(config_data):
                    ...
                    _ = EvaluationTestFactory.create_test_evaluation_config(
                        **config_data
                    )
                else:
                    _ = EvaluationTestFactory.create_test_evaluation_config(
...
class TestEvaluationEdgeCases:
    ...
    def test_zero_length_games(self, tmp_path):
        ...
        try:
            ...
            assert (
                abs((stats.win_rate + stats.loss_rate + stats.draw_rate) - 1.0) < 0.01
            )
    ...
    def test_extreme_game_lengths(self, tmp_path):
        ...
        try:
            ...
            assert 0 <= stats.win_rate <= 1
            assert not (
                stats.avg_game_length < 0 or stats.avg_game_length > 10000
