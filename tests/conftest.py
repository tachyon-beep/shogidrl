...
try:
    if mp.get_start_method(allow_none=True) != 'spawn':
        ...
        print("Successfully set multiprocessing start method to 'spawn' in conftest.py", file=sys.stderr)
except RuntimeError as e:
    print(f"Could not set multiprocessing start method in conftest.py: {e}", file=sys.stderr)
    ...
    try:
        ...
        print("Successfully set multiprocessing start method to 'spawn' in conftest.py (fallback/direct set)", file=sys.stderr)
    except RuntimeError as e_inner:
        print(f"Could not set multiprocessing start method in conftest.py (fallback/direct set): {e_inner}", file=sys.stderr)
