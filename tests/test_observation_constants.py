...
def test_observation_plane_constants_match_implementation():
    ...
    assert obs.shape[0] == expected_num_planes, f"Expected {expected_num_planes} planes, got {obs.shape[0]}"
    ...
    for col in range(9):
        assert obs[OBS_CURR_PLAYER_UNPROMOTED_START, 6, col] > 0.9, f"Black pawn not found at (6,{col})"

    # Similarly, white pawns should be on the opponent's unpromoted plane
    for col in range(9):
        assert obs[OBS_OPP_PLAYER_UNPROMOTED_START, 2, col] > 0.9, f"White pawn not found at (2,{col})"

    # 2. There should be no promoted pieces initially
    num_promoted_types = len(OBS_PROMOTED_ORDER)
    for r in range(9):
        for c in range(9):
            assert np.all(obs[OBS_CURR_PLAYER_PROMOTED_START:OBS_CURR_PLAYER_PROMOTED_START+num_promoted_types, r, c] < 0.1), \
                f"Found promoted piece for current player at ({r},{c})"
            assert np.all(obs[OBS_OPP_PLAYER_PROMOTED_START:OBS_OPP_PLAYER_PROMOTED_START+num_promoted_types, r, c] < 0.1), \
                f"Found promoted piece for opponent at ({r},{c})"

    # 3. Hands should be empty initially
    for i in range(7):
        assert np.all(obs[OBS_CURR_PLAYER_HAND_START+i] < 0.1), f"Current player hand channel {i} not empty"
        assert np.all(obs[OBS_OPP_PLAYER_HAND_START+i] < 0.1), f"Opponent hand channel {i} not empty"

    # 4. Current player indicator should show it's Black's turn (1.0 for Black)
    assert np.all(obs[OBS_CURR_PLAYER_INDICATOR] > 0.9), "Current player indicator should be 1.0 for Black"

    # 5. Move count should be 0 initially (normalized by max_moves_per_game)
    assert np.all(obs[OBS_MOVE_COUNT] < 0.01), "Move count should be close to 0 initially"
