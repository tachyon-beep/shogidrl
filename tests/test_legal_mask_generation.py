...
from keisei.shogi.shogi_core_definitions import Color, PieceType, MoveTuple
...
class TestLegalMaskGeneration: # pylint: disable=too-many-public-methods
    ...
    def test_initial_position_legal_mask(self): # pylint: disable=too-many-statements
        ...
        assert len(legal_moves_tuples) == 30, f"Expected 30 legal moves, got {len(legal_moves_tuples)}. Moves: {legal_moves_tuples}"
        legal_mask = mapper.get_legal_mask(legal_moves_tuples, device=DEVICE)
        assert legal_mask.sum().item() == 30, f"Expected mask sum 30, got {legal_mask.sum().item()}"
    ...
    def test_king_in_check_mask(self): # pylint: disable=too-many-statements
        ...
        expected_king_moves_in_check: Set[MoveTuple] = {
            (8, 4, 8, 3, False), (8, 4, 8, 5, False),  # K to d1, f1
            (8, 4, 7, 3, False), (8, 4, 7, 5, False)   # K to d2, f2
        }
        assert len(legal_moves_tuples) == 4, f"Expected 4 legal moves, got {len(legal_moves_tuples)}. Moves: {legal_moves_tuples}"
        ...
        for move in expected_king_moves_in_check:
            ...
            assert legal_mask[idx].item() is True, f"Expected move {move} to be legal, but it's not in the mask."
        ...
        assert legal_mask[idx_illegal].item() is False, "Move into check (K to e2) should be illegal."
    ...
    def test_checkmate_mask(self): # pylint: disable=too-many-statements
        ...
        assert game.is_in_check(Color.BLACK) is True, "Black should be in check in this position."
        assert game.game_over is True, "Game should be over due to checkmate."

        legal_moves_tuples = game.get_legal_moves()
        assert not legal_moves_tuples, f"Expected no legal moves in checkmate, got {legal_moves_tuples}"

        legal_mask = mapper.get_legal_mask(legal_moves_tuples, device=DEVICE)
        assert legal_mask.sum().item() == 0, "Mask sum should be 0 in checkmate."

    def test_drop_moves_mask(self): # pylint: disable=too-many-locals, too-many-statements # Disabled for now
        ...
        assert len(black_legal_moves) == 78, f"Expected 78 legal moves, got {len(black_legal_moves)}. Moves: {black_legal_moves}"
        assert legal_mask_black.sum().item() == 78, f"Expected mask sum 78, got {legal_mask_black.sum().item()}"
        ...
        assert legal_mask_black[idx_king_move].item() is True, f"Legal king move {king_move} not in mask"
        ...
        assert legal_mask_black[idx_legal_pawn_drop].item() is True, f"Legal pawn drop {legal_pawn_drop} not in mask"

        # Verify an illegal pawn drop (on last rank for Black - row 0)
        illegal_pawn_drop_last_rank: MoveTuple = (None, None, 0, 0, PieceType.PAWN) # Drop to (0,0)
        idx_illegal_drop_last_rank = mapper.shogi_move_to_policy_index(illegal_pawn_drop_last_rank)
        assert legal_mask_black[idx_illegal_drop_last_rank].item() is False, "Illegal pawn drop (last rank) should not be in mask"
        ...
        idx_illegal_drop_occupied = mapper.shogi_move_to_policy_index(illegal_pawn_drop_occupied)
        assert legal_mask_black[idx_illegal_drop_occupied].item() is False, "Illegal pawn drop (occupied) should not be in mask"
        ...
        sfen_nifu_scenario = "P8/9/9/9/4k4/9/9/9/4K4 b P 1" # Black Pawn at (0,0) (SFEN: 1a)
        ...
        nifu_drop_attempt: MoveTuple = (None, None, 1, 0, PieceType.PAWN) # Drop to (1,0)
        ...
        assert nifu_mask[idx_nifu_drop].item() is False, "Illegal pawn drop (Nifu) should not be in mask"
    ...
    def test_promotion_mask(self): # pylint: disable=too-many-statements, too-many-locals
        ...
        sfen_promote_option = "4k4/9/4P4/9/9/9/9/9/4K4 b - 1" # Black Pawn at (2,4) (SFEN e7)
        ...
        move_no_promote: MoveTuple = (2, 4, 1, 4, False) # Corrected: Pawn at (2,4) moves to (1,4)
        move_promote: MoveTuple = (2, 4, 1, 4, True)    # Corrected: Pawn at (2,4) moves to (1,4)
        ...
        assert legal_mask[idx_no_promote].item() is True, f"Non-promoting pawn move (2,4)->(1,4) not in mask. Moves: {legal_moves}"
        assert legal_mask[idx_promote].item() is True, f"Promoting pawn move (2,4)->(1,4) not in mask. Moves: {legal_moves}"
        ...
        king_move_attempt_promote: MoveTuple = (8, 4, 7, 4, True)  # King e1-e2, attempt promote
        idx_king_promote = mapper.shogi_move_to_policy_index(king_move_attempt_promote)
        assert king_mask[idx_king_promote].item() is False, "Illegal king promotion found in mask"


    def test_specific_board_moves_are_legal(self): # pylint: disable=too-many-statements
        ...
        pawn_move: MoveTuple = (6, 6, 5, 6, False) # Corrected: Pawn at (6,6) moves to (5,6)
        idx_pawn = mapper.shogi_move_to_policy_index(pawn_move)
        assert legal_mask[idx_pawn].item() is True, "Legal pawn move 7g7f (6,6)->(5,6) not in mask"
        ...
        assert legal_mask[idx_rook].item() is True, "Legal rook move H8-G8 (7,7)->(7,6) not in mask"
        ...
        assert legal_mask[idx_knight].item() is False, "Illegal knight move N8i-7g (8,1)->(6,2) (blocked by own pawn) should not be in mask"


    def test_no_legal_moves_for_opponent_in_checkmate(self): # pylint: disable=too-many-statements
        ...
        assert game.is_in_check(Color.WHITE) is True, "White should be in check in this position."
        ...
        assert not white_legal_moves, f"White should have no legal moves in checkmate, got {white_legal_moves}"

        legal_mask = mapper.get_legal_mask(white_legal_moves, device=DEVICE)
        assert legal_mask.sum().item() == 0, "Mask sum for White should be 0 in checkmate."
        assert not legal_mask.any(), "No element in the legal mask should be true for White in checkmate."
    ...
    def test_stalemate_mask(self): # pylint: disable=too-many-statements
        ...
        # (0,3), (0,5), (1,3), (1,4), (1,5)
        assert game.is_in_check(Color.BLACK) is False, "King should not be in check in this K vs K setup."
        assert game.game_over is False # Should not be game over by stalemate yet by ShogiGame rules for this position
        assert len(legal_moves) == 5, f"Expected 5 moves, got {len(legal_moves)}. Moves: {legal_moves}"
        # Use legal_mask.sum().item() which returns a Python number
        assert legal_mask.sum().item() == 5, f"Expected mask sum 5, got {legal_mask.sum().item()}" # Black king can move to 5 squares


    def test_nifu_scenario_with_explicit_game_instance(self): # pylint: disable=too-many-statements
        ...
        game_nifu_direct = ShogiGame.from_sfen("4k4/9/9/9/9/9/9/9/4K4 b PP 1") # Black has two pawns
        ...
        assert game_nifu_direct.board[4][0].type == PieceType.PAWN # type, not piece_type
        ...
        game_nifu_direct.current_player = Color.BLACK # Manually switch back for testing black's drop
        ...
        legal_mask_black_nifu = mapper.get_legal_mask(legal_moves_black_nifu, device=DEVICE)
        ...
        assert legal_mask_black_nifu[idx_nifu_drop_attempt].item() is False, \
        ...
        assert move2_nifu_drop not in legal_moves_black_nifu, \
