...
class TestDataFactory:
    ...
    def create_training_session(
        ...
        for _ in range(move_count):
            ...
            think_time = random.uniform(1, 30) if random.random() > 0.1 else random.uniform(30, 120)
            current_time += timedelta(seconds=think_time)
            move_times.append(think_time)

        return {
            'game_state': TestDataFactory.create_game_state(move_count=min(move_count, 30)),
