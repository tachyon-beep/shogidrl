...
def test_reward_stalemate_draw(new_game: ShogiGame):
...
def test_reward_max_moves_draw(new_game: ShogiGame):
...
def test_reward_sennichite_draw(new_game: ShogiGame):
...
def test_make_move_returns_correct_reward_at_terminal_state():
    ...
    # Create a custom implementation to test the return value format
    from unittest.mock import patch

    game = ShogiGame()

    # Mock the get_reward method to return a specific value
    with patch.object(ShogiGame, "get_reward", return_value=1.0):
        # Artificially set up the game to end after making a move
        # by patching apply_move_to_board to set game_over

        orig_apply_move = game.make_move

        def mock_make_move(*args, **kwargs):
        ...
        game.make_move = mock_make_move

        # Now make a move that will trigger our mocked behavior
        move_outcome = game.make_move((6, 6, 5, 6, False))  # Any valid move will do
...
def test_make_move_returns_perspective_specific_reward():
    ...
    move_outcome = game.make_move((2, 4, 1, 4, False))
    ...
    move_outcome = game.make_move((6, 0, 5, 0, False))
