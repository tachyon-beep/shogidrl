...
def sample_observation(minimal_app_config):
    ...
    return torch.randn((2, config.env.input_channels, SHOGI_BOARD_SIZE, SHOGI_BOARD_SIZE))
...
class TestActorCriticBasicFunctionality:
    ...
    @pytest.mark.parametrize("batch_size", [1, 2, 4, 8, 16], ids=["single", "pair", "small", "medium", "large"])
...
class TestActorCriticParameterized:
    ...
    @pytest.mark.parametrize("input_channels", [1, 3, 46, 64], ids=["minimal", "rgb", "default", "large"])
    ...
    @pytest.mark.parametrize("num_actions", [1, 10, 100, 1000], ids=["single", "small", "default", "large"])
...
class TestActorCriticAdvanced:
    ...
    def test_gradient_accumulation(self, minimal_app_config):
        ...
        for name, param in model.named_parameters():
            if param.grad is not None and name in first_grads:
                assert not torch.allclose(param.grad, first_grads[name], rtol=1e-5), \
    ...
    def test_actor_critic_device_compatibility(self, minimal_app_config):
        ...
        # Test CPU
        model_cpu = ActorCritic(input_channels=input_channels, num_actions_total=num_actions)
        ...
        if torch.cuda.is_available():
            model_cuda = ActorCritic(input_channels=input_channels, num_actions_total=num_actions).cuda()
    ...
    def test_actor_critic_serialization(self, minimal_app_config, tmp_path):
        ...
        model_original = ActorCritic(input_channels=input_channels, num_actions_total=num_actions)
        save_path = tmp_path / "test_model.pt"
        torch.save(model_original.state_dict(), save_path)
        
        # Load model and verify
        model_loaded = ActorCritic(input_channels=input_channels, num_actions_total=num_actions)
...
class TestActorCriticEdgeCases:
    ...
    def test_weight_initialization(self, minimal_app_config):
        ...
        for name, param in model.named_parameters():
            if 'weight' in name and param.dim() > 1:  # Skip bias and 1D parameters
                ...
                assert torch.var(param, unbiased=False) > 1e-6, f"Weight parameter {name} has very low variance"
    ...
    def test_output_ranges(self, minimal_app_config):
        ...
        assert torch.all(torch.abs(policy_logits) < 100), "Policy logits are too extreme"
        ...
        assert torch.var(policy_logits, unbiased=False) > 1e-6, "Policy logits have very low variance"
...
class TestActorCriticSerialization:
    ...
    def test_state_dict_save_load(self, minimal_app_config):
        ...
        model1 = ActorCritic(input_channels=input_channels, num_actions_total=num_actions)
        
        # Save state dict
        state_dict = model1.state_dict()
        
        # Create new model and load state dict
        model2 = ActorCritic(input_channels=input_channels, num_actions_total=num_actions)
    ...
    def test_partial_state_dict_loading(self, minimal_app_config):
        ...
        partial_state_dict = {k: v for k, v in original_state_dict.items() if 'conv' in k}
...
class TestActorCriticAdvancedEdgeCases:
    ...
    def test_zero_input(self, minimal_app_config):
        ...
        assert torch.isfinite(policy_logits).all(), "Policy logits contain non-finite values"
    ...
    def test_extreme_input_values(self, minimal_app_config):
        ...
        # Very large inputs
        x_large = torch.full((1, input_channels, SHOGI_BOARD_SIZE, SHOGI_BOARD_SIZE), 1e6)
        policy_large, value_large = model(x_large)
        assert torch.isfinite(policy_large).all(), "Policy logits not finite for large inputs"
        assert torch.isfinite(value_large).all(), "Values not finite for large inputs"
        
        # Very small inputs
        x_small = torch.full((1, input_channels, SHOGI_BOARD_SIZE, SHOGI_BOARD_SIZE), 1e-6)
        policy_small, value_small = model(x_small)
        assert torch.isfinite(policy_small).all(), "Policy logits not finite for small inputs"
    ...
    def test_single_vs_batch_consistency(self, minimal_app_config):
        ...
        for i in range(3):
            torch.testing.assert_close(policy_batch[i:i+1], policy_single, atol=1e-6, rtol=1e-6)
            torch.testing.assert_close(value_batch[i:i+1], value_single, atol=1e-6, rtol=1e-6)
...
class TestActorCriticMemoryEfficiency:
    ...
    def test_gradient_memory_efficiency(self, minimal_app_config):
        ...
        x = torch.randn((2, input_channels, SHOGI_BOARD_SIZE, SHOGI_BOARD_SIZE), requires_grad=True)
        policy, value = model(x)
