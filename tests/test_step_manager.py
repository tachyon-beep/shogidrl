...
from keisei.training.step_manager import StepManager, EpisodeState, StepResult
...
class TestExecuteStep:
    ...
    def test_successful_step_execution(self, step_manager, sample_episode_state, mock_logger, mock_components):
        ...
        log_prob = -1.5
        value_pred = 0.8
        mock_components['agent'].select_action.return_value = (selected_move, policy_index, log_prob, value_pred)
    ...
    def test_make_move_raises_exception(self, step_manager, sample_episode_state, mock_logger, mock_components):
        ...
        selected_move = (1, 2, 3, 4, 5)
        mock_components['agent'].select_action.return_value = (selected_move, 42, -1.5, 0.8)
        ...
        with patch('keisei.training.step_manager.format_move_with_description_enhanced') as mock_format:
            ...
            with patch('time.sleep') as mock_sleep:
                ...
                for call in mock_logger.call_args_list:
                    if "Move" in call[0][0] and "TestPlayer played formatted_move" in call[0][0]:
                        demo_log_found = True
                        break
...
class TestHandleEpisodeEnd:
    ...
    def test_episode_end_white_wins(self, step_manager, sample_episode_state, mock_logger, mock_components):
        ...
        mock_components['game'].reset.return_value = np.random.rand(10, 10, 20).astype(np.float32)
...
class TestUpdateEpisodeState:
    ...
    def test_update_episode_state(self, step_manager, sample_episode_state):
        ...
        assert updated_state.episode_reward == sample_episode_state.episode_reward + 2.5  # 15.5 + 2.5 = 18.0
        assert updated_state.episode_length == sample_episode_state.episode_length + 1    # 25 + 1 = 26
