...
def test_model_save_and_load(tmp_path):
    ...
    policy_output_mapper = PolicyOutputMapper()  # Corrected: No arguments for constructor

    device = config.DEVICE
    agent = PPOAgent(input_channels, policy_output_mapper, device=device)
    # Corrected to use agent.model instead of agent.policy
    original_model_state_dict = {k: v.cpu() for k, v in agent.model.state_dict().items()}
    ...
    loaded_model_state_dict = {k: v.cpu() for k, v in new_agent.model.state_dict().items()}

    # Compare model parameters
    for key in original_model_state_dict:
        assert torch.equal(original_model_state_dict[key], loaded_model_state_dict[key]), \
    ...
    if hasattr(third_agent.model, 'conv') and hasattr(third_agent.model.conv, 'weight'):
    ...
    elif hasattr(third_agent.model, 'policy_head') and hasattr(third_agent.model.policy_head, 'weight'):  # Fallback to policy_head if conv not found
        ...
        print("Warning: Could not find a suitable layer to modify for testing model loading into a modified agent.")

    third_agent.load_model(model_path)
    # Corrected to use third_agent.model
    third_loaded_model_state_dict = {k: v.cpu() for k, v in third_agent.model.state_dict().items()}
    for key in original_model_state_dict:
        assert torch.equal(original_model_state_dict[key], third_loaded_model_state_dict[key]), \
