...
from keisei.utils.profiling import (
    perf_monitor, profile_function, profile_training_step, 
    profile_game_operation, profile_code_block
)
...
class TestRemediationIntegration:
    ...
    def test_error_handling_across_components(self):
        ...
        stats = perf_monitor.get_stats()
        function_key = f"{operation_with_error.__module__}.{operation_with_error.__name__}"
...
class TestBackwardCompatibility:
    ...
    def test_existing_config_system(self):
        ...
        assert hasattr(config, 'total_timesteps')  # Fixed: max_episodes -> total_timesteps
...
class TestPerformanceImpact:
    ...
    def test_seeding_performance_impact(self):
        ...
        assert overhead_ratio < 1.5, f"Seeding adds too much overhead: {overhead_ratio:.2f}x"
    ...
    def test_profiling_performance_impact(self):
        ...
        assert overhead_ratio < 5.0, f"Profiling overhead too high: {overhead_ratio:.2f}x"
...
class TestRealWorldScenarios:
    ...
    def test_development_workflow(self):
        ...
        def evaluate_position():
            state = env_manager.game.get_observation()  # Fixed: get_state -> get_observation
            time.sleep(0.002)
            return 0.5
        ...
        evaluate_position_keys = [k for k in stats.keys() if "evaluate_position" in k and k.endswith("_count")]
        assert len(evaluate_position_keys) > 0, f"Should have evaluate_position stats, found keys: {list(stats.keys())}"
        # training_steps_completed counter is only present when using @profile_training_step
        assert stats.get("training_steps_completed", 0) == 0  # No training steps in this scenario
    ...
    def test_debugging_scenario(self):
        ...
        import numpy as np
        assert np.array_equal(state, state2), "States should be consistent due to seeding"
    ...
    def test_performance_optimization_workflow(self):
        ...
        baseline_avg = baseline_stats[f"{baseline_operation.__module__}.{baseline_operation.__name__}_avg"]
        optimized_avg = optimized_stats[f"{optimized_operation.__module__}.{optimized_operation.__name__}_avg"]
