...
class TestDependencyVersioning:
    ...
    def test_numpy_version_compatibility(self):
        ...
        assert version_parts[0] > 1 or (version_parts[0] == 1 and version_parts[1] >= 24), \
...
class TestRemovedDependencies:
    ...
    def test_matplotlib_references_removed(self):
        ...
        assert 'matplotlib' not in main_deps_section.lower(), \
            "matplotlib found in main dependencies"
        
        # Should also not be in dev dependencies
        dev_deps_section = content.split('[project.optional-dependencies]')[1] if '[project.optional-dependencies]' in content else ""
        assert 'matplotlib' not in dev_deps_section.lower(), \
...
class TestDependencyAnalysis:
    ...
    def test_deptry_analysis(self):
        ...
        try:
            ...
            assert result.returncode in [0, 1], f"deptry failed with code {result.returncode}"
            
            # Should not mention matplotlib
            assert "matplotlib" not in result.stdout.lower(), \
        ...
        for file_path in core_files:
            ...
            for import_line in import_lines:
                if 'matplotlib' in import_line:
                    pytest.fail(f"Found matplotlib import in {file_path}: {import_line}")
    ...
    def test_pip_environment_consistency(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                required_packages = {
                    "torch", "numpy", "pyyaml", "rich", 
                    "pydantic", "python-dotenv", "wandb"
                }
                
                for pkg in required_packages:
                    assert pkg in package_names or pkg.replace("-", "_") in package_names, \
        ...
        if requirements_path.exists():
            ...
            for pkg in main_packages:
                if pkg in pyproject_content:
                    ...
                    assert pkg in requirements_content or pkg.replace("-", "_") in requirements_content, \
