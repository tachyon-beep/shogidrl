...
def test_get_observation_initial_state_dimensions(new_game: ShogiGame):
    ...
    assert obs.shape == (46, 9, 9), "Observation shape is incorrect based on shogi_game_io.py"
...
def test_get_observation_hand_pieces_black_one_pawn(game_with_black_pawn_in_hand: ShogiGame):
    ...
    assert np.allclose(obs[pawn_hand_channel_index], expected_value), \
    ...
    for i, pt in enumerate(hand_types_order):
        if pt != PieceType.PAWN:
            channel_idx = 28 + i
            assert np.all(obs[channel_idx] == 0.0), f"Black hand plane for {pt.name} should be 0. Got {obs[channel_idx][0][0]}"
...
def test_get_observation_hand_pieces_white_one_rook(game_with_white_rook_in_hand: ShogiGame):
    ...
    expected_value = 1 / 18.0
    assert np.allclose(obs[rook_hand_channel_index], expected_value), \
    ...
    for i, pt in enumerate(hand_types_order):
        if pt != PieceType.ROOK:
            channel_idx = 35 + i
            assert np.all(obs[channel_idx] == 0.0), f"White hand plane for {pt.name} should be 0. Got {obs[channel_idx][0][0]}"
...
def test_get_observation_multiple_hand_pieces_mixed_players(game_with_mixed_hands: ShogiGame):
    ...
    assert np.allclose(obs[pawn_idx_black], expected_pawn_black), f"Black 3 pawns hand plane incorrect. Expected {expected_pawn_black}, got {obs[pawn_idx_black][0][0]}"
    assert np.allclose(obs[gold_idx_black], expected_gold_black), f"Black 1 gold hand plane incorrect. Expected {expected_gold_black}, got {obs[gold_idx_black][0][0]}"
    ...
    assert np.allclose(obs[bishop_idx_white], expected_bishop_white), f"White 2 bishops hand plane incorrect. Expected {expected_bishop_white}, got {obs[bishop_idx_white][0][0]}"
    assert np.allclose(obs[silver_idx_white], expected_silver_white), f"White 1 silver hand plane incorrect. Expected {expected_silver_white}, got {obs[silver_idx_white][0][0]}"
...
def test_get_observation_current_player_plane_black_turn(new_game: ShogiGame):
    ...
    assert np.all(obs[current_player_plane_index] == 1.0), "Current player plane incorrect for Black's turn (should be 1.0)"
...
def test_get_observation_current_player_plane_white_turn(new_game: ShogiGame):
    ...
    obs = new_game.get_observation()
    current_player_plane_index = 42
    assert np.all(obs[current_player_plane_index] == 0.0), "Current player plane incorrect for White's turn (should be 0.0)"
...
def test_get_observation_move_count_plane(game_with_move_count_5: ShogiGame):
    ...
    assert np.allclose(obs[move_count_plane_index], expected_value), \
...
def test_undo_move_simple_board_move(new_game: ShogiGame):
    ...
    assert game.current_player == initial_player, "Current player not restored after undo"
...
def test_undo_move_capture(new_game: ShogiGame):
    ...
    assert game.hands[Color.BLACK.value][PieceType.PAWN] == initial_black_hand.get(PieceType.PAWN, 0) + 1
    ...
    assert game.hands[Color.BLACK.value] == initial_black_hand, "Black's hand not restored"
    assert game.hands[Color.WHITE.value] == initial_white_hand, "White's hand not restored (should be unchanged)"
...
def test_undo_move_drop(new_game: ShogiGame):
    ...
    assert game.move_count == initial_move_count, "Move count not restored"
    assert game.hands[Color.BLACK.value] == initial_black_hand, "Black's hand not restored"
    assert game.get_piece(4,4) is None, "Dropped piece not removed from board after undo"
...
def test_undo_move_promotion_with_capture(new_game: ShogiGame):
    ...
    assert game.hands[Color.BLACK.value][PieceType.LANCE] == initial_black_hand.get(PieceType.LANCE, 0) + 1
    ...
    assert game.move_count == initial_move_count, "Move count not restored"
    assert game.hands[Color.BLACK.value] == initial_black_hand, "Black's hand not restored"
    assert game.hands[Color.WHITE.value] == initial_white_hand, "White's hand not restored"
    ...
    for r in range(9):
        for c in range(9):
            if (r,c) not in [(1,2), (0,2)]:
                game.set_piece(r,c,None)
    game.set_piece(8,8, Piece(PieceType.KING, Color.BLACK))
    game.set_piece(0,8, Piece(PieceType.KING, Color.WHITE)) # King away from promotion square
...
def test_undo_move_multiple_moves(new_game: ShogiGame):
    ...
    assert game.hands[Color.BLACK.value].get(PieceType.PAWN, 0) == initial_black_hand.get(PieceType.PAWN, 0) + 1 # Captured white pawn
    ...
    assert game.current_player == Color.BLACK # Should be Black's turn again (was White's before undo)
    assert game.hands[Color.BLACK.value].get(PieceType.PAWN, 0) == initial_black_hand.get(PieceType.PAWN, 0)
    ...
    assert game.current_player == Color.WHITE # Should be White's turn (was Black's before undo)
    ...
    assert game.get_piece(5,6) is None
    assert game.current_player == Color.BLACK # Should be Black's turn (was White's before undo)
    ...
    assert current_board_str == initial_board_str, \
        f"Board state not fully restored after multiple undos.\nExpected:\n{initial_board_str}\nGot:\n{current_board_str}"
    
    assert game.current_player == initial_player, "Current player not restored after multiple undos"
    assert game.move_count == initial_move_count, "Move count not restored after multiple undos"
    assert game.hands[Color.BLACK.value] == initial_black_hand, "Black's hand not restored after multiple undos"
    assert game.hands[Color.WHITE.value] == initial_white_hand, "White's hand not restored after multiple undos"
