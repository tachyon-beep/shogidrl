...
def test_policy_output_mapper_mappings(mapper: PolicyOutputMapper):
    ...
    assert idx3 == 12960 + 1 # (0*9+0)*7 + 1 (Lance is index 1 in droppable_piece_types)
    ...
    with pytest.raises(IndexError):
        mapper.policy_index_to_shogi_move(mapper.get_total_actions()) # Exact total_actions is out of bounds (0-indexed)
    with pytest.raises(IndexError):
...
def test_get_legal_mask(mapper: PolicyOutputMapper):
    ...
    for move in legal_shogi_moves:
        assert move in mapper.move_to_idx, f"Test move {move} is not in the mapper. Check move generation or test data."
