...
class TestPPOAgentErrorHandling:
    ...
    def test_select_action_invalid_observation_shape(self, ppo_agent_basic, dummy_legal_mask):
        ...
        for invalid_obs in [invalid_obs_1d, invalid_obs_2d, invalid_obs_4d]:
            with pytest.raises((RuntimeError, ValueError, AssertionError)):
                ppo_agent_basic.select_action(invalid_obs, dummy_legal_mask, is_training=True)
                
    def test_select_action_invalid_legal_mask_shape(self, ppo_agent_basic, dummy_observation):
        ...
        for invalid_mask in [invalid_mask_small, invalid_mask_large]:
            with pytest.raises((RuntimeError, ValueError, IndexError)):
                ppo_agent_basic.select_action(dummy_observation, invalid_mask, is_training=True)
    ...
    def test_select_action_all_illegal_moves(self, ppo_agent_basic, dummy_observation):
        ...
        all_illegal_mask = torch.zeros(ppo_agent_basic.num_actions_total, dtype=torch.bool, device="cpu")
...
class TestPPOAgentLegalMaskEdgeCases:
    ...
    def test_single_legal_action(self, ppo_agent_basic, dummy_observation):
        ...
        single_legal_mask = torch.zeros(ppo_agent_basic.num_actions_total, dtype=torch.bool, device="cpu")
    ...
    def test_legal_mask_device_mismatch(self, ppo_agent_basic, dummy_observation):
        ...
        if torch.cuda.is_available():
            ...
            wrong_device_mask = torch.ones(ppo_agent_basic.num_actions_total, dtype=torch.bool, device="cuda")
            
            # Should either handle gracefully or give clear error
            with pytest.raises((RuntimeError, ValueError)):
                ppo_agent_basic.select_action(dummy_observation, wrong_device_mask, is_training=True)
        else:
    ...
    def test_legal_mask_wrong_dtype(self, ppo_agent_basic, dummy_observation):
        ...
        float_mask = torch.ones(ppo_agent_basic.num_actions_total, dtype=torch.float32, device="cpu")
...
class TestPPOAgentModelPersistence:
    ...
    def test_save_load_round_trip_preserves_behavior(self, ppo_agent_basic, dummy_observation, dummy_legal_mask):
        """Test that save/load preserves agent behavior."""
        # Get initial action selection
        initial_move, initial_idx, initial_log_prob, initial_value = ppo_agent_basic.select_action(
            dummy_observation, dummy_legal_mask, is_training=False  # Deterministic for consistency
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            loaded_move, loaded_idx, loaded_log_prob, loaded_value = ppo_agent_basic.select_action(
    ...
    def test_save_to_invalid_path(self, ppo_agent_basic):
        ...
        invalid_path = "/root/cannot_write_here/model.pth"  # Assuming no write permission
...
class TestPPOAgentConfigurationValidation:
    ...
    def test_invalid_hyperparameters(self, ppo_test_model):
        ...
        for invalid_config in invalid_configs:
            ...
            from keisei.config_schema import AppConfig, EnvConfig, EvaluationConfig, LoggingConfig, WandBConfig, DemoConfig, ParallelConfig
            
            app_config = AppConfig(
                env=EnvConfig(device="cpu", input_channels=46, num_actions_total=13527, seed=42, max_moves_per_game=200),
                ...
                logging=LoggingConfig(log_file="test.log", model_dir="/tmp/test_models", run_name="test_run"),
                wandb=WandBConfig(
                    enabled=False,
                ...
                parallel=ParallelConfig(enabled=False, num_workers=4, batch_size=32, sync_interval=100, compression_enabled=True, timeout_seconds=10.0, max_queue_size=1000, worker_seed_offset=1000),
            )
            
            # Instantiate PPOAgent to test config integration
            agent = PPOAgent(model=ppo_test_model, config=app_config, device=torch.device("cpu"))
...
class TestPPOAgentDevicePlacement:
    ...
    def test_cpu_device_consistency(self, ppo_agent_basic):
        ...
        dummy_mask = torch.ones(ppo_agent_basic.num_actions_total, dtype=torch.bool, device="cpu")
    ...
    def test_cuda_device_consistency(self, minimal_app_config, ppo_test_model):
        ...
        dummy_mask = torch.ones(agent.num_actions_total, dtype=torch.bool, device="cuda")
    ...
    def test_mixed_device_inputs(self, ppo_agent_basic):
        ...
        if torch.cuda.is_available():
            ...
            cpu_mask = torch.ones(ppo_agent_basic.num_actions_total, dtype=torch.bool, device="cpu")
...
class TestPPOAgentBoundaryConditions:
    ...
    def test_extremely_small_learning_rate(self, minimal_app_config, ppo_test_model):
        ...
        experience_buffer = ExperienceBuffer(buffer_size=4, gamma=0.99, lambda_gae=0.95, device="cpu")
    ...
    def test_zero_experiences_after_compute_advantages(self, ppo_agent_basic):
        ...
        experience_buffer = ExperienceBuffer(buffer_size=2, gamma=0.99, lambda_gae=0.95, device="cpu")
...
class TestPPOAgentSchedulerEdgeCases:
    ...
    def test_scheduler_extreme_learning_rates(self, minimal_app_config, ppo_test_model):
        ...
        config.training.lr_schedule_kwargs = {"gamma": 0.9}  # Aggressive but not extreme decay
    ...
    def test_scheduler_with_very_large_total_steps(self, minimal_app_config, ppo_test_model):
    ...
    def test_scheduler_boundary_learning_rates(self, minimal_app_config, ppo_test_model):
        ...
        test_cases = [
            ...
            {"initial_lr": 3e-4, "schedule_type": "step", "kwargs": {"step_size": 1, "gamma": 0.1}},
        ]
    ...
    def test_scheduler_configuration_edge_cases(self, minimal_app_config, ppo_test_model):
        ...
        edge_cases = [
            ...
            # Conflicting kwargs
            {"lr_schedule_type": "cosine", "lr_schedule_kwargs": {"T_max": 10, "eta_min_fraction": 0.1}},
        ]
