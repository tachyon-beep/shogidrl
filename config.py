...
PPO_EPOCHS = (
    10  # Number of optimization epochs over the collected batch per PPO update.
)
...
ENTROPY_COEFF = (
    0.01  # Coefficient for the entropy bonus in the PPO loss, encouraging exploration.
)
...
MAX_MOVES_PER_GAME_EVAL = (
    256  # Maximum moves for evaluation games (can differ from training).
)

# --- Model Saving & Checkpointing ---
SAVE_FREQ_EPISODES = (
    200  # Frequency (in episodes) for saving model checkpoints during training.
)

# --- Periodic Evaluation Settings (during training) ---
EVAL_DURING_TRAINING = (
    True  # Master switch to enable/disable periodic evaluation during training.
)
...
EVAL_FREQ_EPISODES = (
    100  # If a separate evaluation frequency independent of saving is ever needed.
)
EVAL_NUM_GAMES = 10  # Number of games to play during each periodic evaluation session.
EVAL_OPPONENT_TYPE = (
    "heuristic"  # Opponent for periodic evals: "random", "heuristic", or "ppo".
)
...
WANDB_LOG_TRAIN = (
    True  # Master switch to enable W&B logging for the main training process.
)
...
EVAL_WANDB_PROJECT = (
    "shogi-drl-periodic-evaluation"  # W&B project for periodic evaluation runs.
)
EVAL_WANDB_ENTITY = (
    None  # W&B entity for periodic evaluation runs. None uses W&B default.
)
EVAL_WANDB_RUN_NAME_PREFIX = (
    "periodic_eval_"  # Prefix for W&B run names for evaluation runs.
)

# --- Debugging & Display Settings ---
ENABLE_DEMO_MODE = (
    True  # If True, slows down training turns for easier real-time observation.
)
DEMO_MODE_DELAY = 0.5  # Delay in seconds between moves when ENABLE_DEMO_MODE is True.
