#!/usr/bin/env python3
"""
Comprehensive Integration Testing for Keisei Evaluation System

This test suite verifies all integration points between the evaluation system 
and the broader Keisei ecosystem following the completion drive methodology.
"""
...
def test_training_callback_integration():
    ...
    try:
        ...
        mock_trainer.evaluation_manager.evaluate_current_agent.return_value = {
            "summary_stats": Mock(win_rate=0.6, loss_rate=0.3)
        }
        ...
        # Test callback doesn't trigger before interval
        callback.on_step_end(mock_trainer)
        assert not mock_trainer.evaluation_manager.evaluate_current_agent.called
        
        # Advance timestep to trigger evaluation
        mock_trainer.metrics_manager.global_timestep = 50000
        callback.on_step_end(mock_trainer)
        
        # Verify evaluation was triggered
        assert mock_trainer.evaluation_manager.evaluate_current_agent.called
        assert mock_trainer.agent.model.eval.called
        assert mock_trainer.agent.model.train.called
...
def test_move_validation_integration():
    ...
    try:
        ...
        assert hasattr(game, 'get_legal_actions')
...
def test_metrics_integration():
    ...
    try:
        from keisei.evaluation.core import EvaluationResult, SummaryStats, GameResult
        from datetime import datetime
        ...
        assert result.strategy == "single_opponent"
...
def main():
    ...
    print("🔧 KEISEI EVALUATION SYSTEM COMPREHENSIVE INTEGRATION TESTING")
    print("="*70)
