...
class EnhancedEvaluationManager(EvaluationManager):
    ...
    def _initialize_enhanced_features(
        ...
        if self.enable_background_tournaments:
            try:
                ...
                self.background_tournament_manager = BackgroundTournamentManager(
                    ...
                    or self._default_progress_callback,
        ...
        if self.enable_advanced_analytics:
            try:
                ...
                self.advanced_analytics = AdvancedAnalytics(
                    significance_level=0.05, min_practical_difference=0.05
                )
        ...
        if self.enable_enhanced_opponents:
            try:
                ...
                opponent_data_file = (
                    self.analytics_output_dir / "opponent_performance_data.json"
                )
                self.enhanced_opponent_manager = EnhancedOpponentManager(
                    opponent_data_file=opponent_data_file, target_win_rate=0.55
                )
    ...
    def _default_progress_callback(self, progress):
        ...
        if progress.is_complete:
            logger.info(
                f"Tournament {progress.tournament_id} completed with status: {progress.status.value}"
            )
    ...
    def generate_analysis_report(
        ...
        try:
            if output_file is None:
                ...
                output_file = (
                    self.analytics_output_dir / f"analysis_report_{timestamp}.json"
                )
    ...
    def compare_performance(
        ...
        if not self.advanced_analytics:
            logger.warning(
                "Advanced analytics not available for performance comparison"
            )
            return None
        ...
        try:
            ...
            logger.info(
                f"Performance comparison completed: {comparison.recommendation}"
            )
            return comparison
    ...
    def select_adaptive_opponent(
        self, current_win_rate: Optional[float] = None, strategy: Optional[str] = None
        ...
        try:
            ...
            selection_strategy = strategy_map.get(
                strategy, SelectionStrategy.ADAPTIVE_DIFFICULTY
            )

            opponent = self.enhanced_opponent_manager.select_opponent(
                strategy=selection_strategy, agent_current_win_rate=current_win_rate
            )

            if opponent:
                logger.debug(
                    f"Selected opponent: {opponent.name} using strategy: {strategy}"
                )
