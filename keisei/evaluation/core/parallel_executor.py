...
class ParallelGameExecutor:
    ...
    async def execute_games_parallel(
        ...
        logger.info(f"Starting parallel execution of {len(tasks)} games with max {self.max_concurrent_games} concurrent")
        ...
        try:
            ...
            completed_count = 0
            for future in as_completed(future_to_task.keys(), timeout=self.timeout_per_game_seconds * len(tasks)):
        ...
        logger.info(f"Parallel execution completed: {len(successful_results)} successful, {len(error_messages)} failed")
    ...
    def _execute_single_game(self, task: ParallelGameTask) -> Optional[GameResult]:
        ...
        try:
            ...
            use_in_memory = getattr(task.context.configuration, 'enable_in_memory_evaluation', False)
            
            if use_in_memory and hasattr(task.game_executor, '__self__') and hasattr(task.game_executor.__self__, 'evaluate_step_in_memory'):
                # Use in-memory evaluation if available
                logger.debug(f"Using in-memory evaluation for game {task.task_id}")
                result = asyncio.run(task.game_executor.__self__.evaluate_step_in_memory(
            ...
            task.error = e
            logger.error(f"Error executing game {task.task_id}: {str(e)}", exc_info=True)
            return None
...
class BatchGameExecutor:
    ...
    async def execute_games_in_batches(
        ...
        logger.info(f"Executing {len(tasks)} games in {len(batches)} batches of size {self.batch_size}")
        
        completed_games = 0
        total_games = len(tasks)
        
        for batch_idx, batch_tasks in enumerate(batches):
            logger.info(f"Executing batch {batch_idx + 1}/{len(batches)} with {len(batch_tasks)} games")
        ...
        logger.info(f"All batches completed: {len(all_results)} successful games, {len(all_errors)} errors")
