...
class BackgroundTournamentManager:
    ...
    async def start_tournament(
        ...
        total_games = len(opponents) * getattr(tournament_config, 'num_games_per_opponent', 2)
        ...
        def task_done_callback(task):
            if task.exception():
                logger.error(f"Tournament {tournament_id} task failed: {task.exception()}")
    ...
    async def _execute_tournament(
        ...
        try:
            async with self._semaphore:
                await self._update_progress(tournament_id, status=TournamentStatus.RUNNING)
                progress.start_time = datetime.now()
                
                # Delay to ensure tournament is in RUNNING state for tests
                await asyncio.sleep(0.15)
                
                logger.info(f"Executing tournament {tournament_id} with {len(opponents)} opponents")
                ...
                progress.standings = result.analytics_data.get('tournament_specific_analytics', {})
                progress.end_time = datetime.now()
                
                await self._update_progress(tournament_id, status=TournamentStatus.COMPLETED)
            ...
            await self._update_progress(tournament_id, status=TournamentStatus.CANCELLED)
    ...
    async def _execute_with_progress_tracking(
        ...
        for round_idx, opponent in enumerate(opponents):
            ...
            try:
                ...
                logger.error(f"Error in round {round_idx + 1} of tournament {tournament_id}: {e}")
    ...
    def get_tournament_progress(self, tournament_id: str) -> Optional[TournamentProgress]:
    ...
    def list_active_tournaments(self) -> List[TournamentProgress]:
        ...
        return [
            progress for progress in self._active_tournaments.values()
