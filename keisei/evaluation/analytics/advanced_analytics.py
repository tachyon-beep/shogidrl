...
class AdvancedAnalytics:
    ...
    def compare_performance(
        ...
        comparison_win_rate = comparison_wins / comparison_total if comparison_total > 0 else 0
        
        win_rate_difference = comparison_win_rate - baseline_win_rate
        win_rate_change_percent = (win_rate_difference / baseline_win_rate * 100) if baseline_win_rate > 0 else float('inf')
        ...
        if baseline_total > 0 and comparison_total > 0:
            z_test = self._two_proportion_z_test(
                ...
                comparison_wins, comparison_total
            )
        ...
        practical_significance = abs(win_rate_difference) >= self.min_practical_difference
    ...
    def analyze_trends(
        ...
        if SCIPY_AVAILABLE:
            ...
            if n * sum_x2 - sum_x * sum_x == 0:
                ...
                ss_res = sum((values[i] - (slope * days[i] + intercept)) ** 2 for i in range(n))
    ...
    def _generate_automated_insights(self, report: Dict[str, Any]) -> List[str]:
        ...
        if avg_length > 100:
            insights.append("🔍 Games are running long. Consider investigating efficiency.")
        elif avg_length < 20:
        ...
        if "advanced_metrics" in report and "streaks" in report["advanced_metrics"]:
            ...
            if max_loss_streak > 5:
                insights.append(f"📉 Concerning loss streak of {max_loss_streak} games.")
