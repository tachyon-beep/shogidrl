...
class EnvConfig(BaseModel):
    ...
    input_channels: int = Field(46, description="Number of input channels for the neural network.")
    num_actions_total: int = Field(13527, description="Total number of possible actions.")
    seed: int = Field(42, description="Random seed for reproducibility.")
    max_moves_per_game: int = Field(500, description="Maximum number of moves per game before declaring a draw.")


class TrainingConfig(BaseModel):
    total_timesteps: int = Field(500_000, description="Total number of environment steps for training.")
    ...
    enable_spinner: bool = Field(True, description="Enable spinner column in progress bar (looks cool!).")
    ...
    weight_decay: float = Field(0.0, description="Weight decay (L2 regularization) for optimizer.")
...
class EvaluationConfig(BaseModel):
    enable_periodic_evaluation: bool = Field(True, description="Enable periodic evaluation during training.")
    evaluation_interval_timesteps: int = Field(50000, description="Run evaluation every N timesteps.")
    num_games: int = Field(20, description="Number of games to play during evaluation.")
    opponent_type: str = Field("random", description="Type of opponent: 'random', 'heuristic', etc.")
    max_moves_per_game: int = Field(500, description="Maximum number of moves per evaluation game.")
    log_file_path_eval: str = Field("eval_log.txt", description="Path for the evaluation log file.")
    wandb_log_eval: bool = Field(False, description="Enable Weights & Biases logging for evaluation.")
    elo_registry_path: Optional[str] = Field("elo_ratings.json", description="Path to Elo registry JSON file")
    agent_id: Optional[str] = Field(None, description="Identifier for the evaluated model")
    opponent_id: Optional[str] = Field(None, description="Identifier for the opponent model")
...
class LoggingConfig(BaseModel):
    log_file: str = Field("logs/training_log.txt", description="Path for the main training log.")
    model_dir: str = Field("models/", description="Directory to save model checkpoints.")
...
class WandBConfig(BaseModel):
    ...
    log_model_artifact: bool = Field(False, description="Enable logging model artifacts to W&B.")


class ParallelConfig(BaseModel):
    enabled: bool = Field(False, description="Enable parallel experience collection.")
    num_workers: int = Field(4, description="Number of parallel workers for experience collection.")
    batch_size: int = Field(32, description="Batch size for experience transmission from workers.")
    sync_interval: int = Field(100, description="Steps between model weight synchronization.")
    compression_enabled: bool = Field(True, description="Enable compression for model weight transmission.")
    timeout_seconds: float = Field(10.0, description="Timeout for worker communication operations.")
    max_queue_size: int = Field(1000, description="Maximum size of experience queues.")
    worker_seed_offset: int = Field(1000, description="Offset for worker random seeds to ensure diversity.")
...
class DemoConfig(BaseModel):
    ...
    demo_mode_delay: float = Field(0.5, description="Delay in seconds between moves in demo mode.")
