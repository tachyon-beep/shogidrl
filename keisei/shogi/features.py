    ...
    for r in range(9):
        for c in range(9):
            ...
            if not piece.is_promoted():
                if is_curr:
                    idx = (
                        game.OBS_CURR_PLAYER_UNPROMOTED_START
                        + game.OBS_UNPROMOTED_ORDER.index(piece.piece_type)
                    )
                else:
                    idx = (
                        game.OBS_OPP_PLAYER_UNPROMOTED_START
                        + game.OBS_UNPROMOTED_ORDER.index(piece.piece_type)
                    )
            else:
                # Promoted
                if is_curr:
                    idx = (
                        game.OBS_CURR_PLAYER_PROMOTED_START
                        + game.OBS_PROMOTED_ORDER.index(piece.piece_type)
                    )
                else:
                    idx = (
                        game.OBS_OPP_PLAYER_PROMOTED_START
                        + game.OBS_PROMOTED_ORDER.index(piece.piece_type)
                    )
            obs[idx, r, c] = 1.0
    # --- Hand pieces ---
    for i, pt in enumerate(game.OBS_UNPROMOTED_ORDER):
        obs[game.OBS_CURR_PLAYER_HAND_START + i, :, :] = game.hands[
            game.current_player
        ].get(pt, 0)
        opp = (
            game.current_player.opponent()
            if hasattr(game.current_player, "opponent")
            else (1 - game.current_player)
        )
