...
class PerformanceBenchmarker:
    ...
    def benchmark_model(
        ...
        result = BenchmarkResult(
            ...
            std_time_ms=statistics.stdev(filtered_timings) if len(filtered_timings) > 1 else 0.0,
            min_time_ms=min(filtered_timings),
            max_time_ms=max(filtered_timings),
            memory_peak_mb=max(memory_peaks) if memory_peaks else 0.0,
            memory_allocated_mb=statistics.mean(memory_allocated) if memory_allocated else 0.0,
    ...
    def validate_numerical_equivalence(
        ...
        with torch.no_grad():
            for i in range(num_samples):
                ...
                policy_diff = torch.max(torch.abs(baseline_policy - optimized_policy)).item()
                value_diff = torch.max(torch.abs(baseline_value - optimized_value)).item()
...
def create_benchmarker(
    ...
    return PerformanceBenchmarker(
        ...
        enable_profiling=getattr(config_training, 'enable_compilation_benchmarking', True),
