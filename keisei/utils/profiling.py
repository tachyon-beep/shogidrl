...
class PerformanceMonitor:
    ...
    def print_summary(self):
        ...
        for key in stats.keys():
            if any(key.endswith(suffix) for suffix in ['_avg', '_min', '_max', '_total', '_count']):
        ...
        # Show counters
        counters = {k: v for k, v in stats.items() if k not in {f"{op}_{suffix}" for op in timing_ops for suffix in ['avg', 'min', 'max', 'total', 'count']}}
        if counters:
            print("\nCounters:")
