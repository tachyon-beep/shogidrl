...
from keisei.utils.performance_benchmarker import PerformanceBenchmarker, ComparisonResult
...
class CompilationResult:
    ...
    def __str__(self) -> str:
        if self.success:
            perf_str = f", {self.performance_improvement:.2f}x speedup" if self.performance_improvement else ""
...
class CompilationValidator:
    ...
    def __init__(
        ...
        self.enable_fallback = getattr(config_training, 'enable_compilation_fallback', True)
        self.validate_output = getattr(config_training, 'validate_compiled_output', True)
        self.tolerance = getattr(config_training, 'compilation_validation_tolerance', 1e-5)
        self.warmup_steps = getattr(config_training, 'compilation_warmup_steps', 5)
        self.enable_benchmarking = getattr(config_training, 'enable_compilation_benchmarking', True)
