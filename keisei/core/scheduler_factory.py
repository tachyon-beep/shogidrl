...
class SchedulerFactory:
    ...
    @staticmethod
    def _create_linear_scheduler(optimizer: torch.optim.Optimizer, total_steps: int, kwargs: Dict[str, Any]) -> LambdaLR:
    ...
    @staticmethod
    def _create_cosine_scheduler(optimizer: torch.optim.Optimizer, total_steps: int, kwargs: Dict[str, Any]) -> CosineAnnealingLR:
    ...
    @staticmethod
    def _create_exponential_scheduler(optimizer: torch.optim.Optimizer, kwargs: Dict[str, Any]) -> ExponentialLR:
        """Create exponential decay scheduler."""
        gamma = kwargs.get("gamma", 0.995)
        return ExponentialLR(optimizer, gamma=gamma)

    @staticmethod
    def _create_step_scheduler(optimizer: torch.optim.Optimizer, kwargs: Dict[str, Any]) -> StepLR:
