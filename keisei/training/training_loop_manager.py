...
class TrainingLoopManager:
        ...
        try:
            while self.trainer.global_timestep < self.config.training.total_timesteps:
                ...
                if self.trainer.global_timestep >= self.config.training.total_timesteps:
                    log_both(f"Target timesteps ({self.config.training.total_timesteps}) reached during epoch {self.current_epoch}.")
                    break
            ...
            log_both("Training interrupted by user (KeyboardInterrupt in TrainingLoopManager).", also_to_wandb=True)
            raise 
    ...
    def _run_epoch(self, log_both):
        ...
        while num_steps_collected_this_epoch < self.config.training.steps_per_epoch:
            ...
            if self.episode_state is None: 
                log_both("[ERROR] Episode state is None at the start of a step collection. Resetting.", also_to_wandb=True)
                self.episode_state = self.step_manager.reset_episode()
                if self.episode_state is None: 
                    raise RuntimeError("Failed to recover from None episode_state by resetting.")
                continue 
            ...
            if not step_result.success:
                log_both(f"[WARNING] Step execution failed at timestep {self.trainer.global_timestep}. Resetting episode.", also_to_wandb=True)
                self.episode_state = self.step_manager.reset_episode()
                continue 

            updated_episode_state = self.step_manager.update_episode_state(self.episode_state, step_result)

            if step_result.done:
                if step_result.info and "winner" in step_result.info:
                    winner = step_result.info["winner"]
                    if winner == "black": self.trainer.black_wins += 1
                    elif winner == "white": self.trainer.white_wins += 1
                    else: self.trainer.draws += 1
                else:
                ...
                total_games = self.trainer.black_wins + self.trainer.white_wins + self.trainer.draws
                bw_rate = (self.trainer.black_wins / total_games if total_games > 0 else 0.0)
                ww_rate = (self.trainer.white_wins / total_games if total_games > 0 else 0.0)
            ...
            if (self.trainer.global_timestep % self.config.training.render_every_steps == 0):
                if hasattr(self.display, 'update_log_panel') and callable(self.display.update_log_panel):
            ...
            if (current_time - self.last_display_update_time) > display_update_interval :
                ...
                self.trainer.pending_progress_updates.setdefault("current_epoch", self.current_epoch)
